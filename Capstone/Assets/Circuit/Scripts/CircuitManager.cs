...
/*            Debug.Log("병렬 연결 저항의 합 : " + 1/sumOfParallelR);*/
    ...
    // 회로의 전체 저항을 구하는 함수 (순환 호출로 순회) - 작성중인 수정본
    public double calcEntireR(ComponentClass root, ComponentClass node, List<List<double>> nextOfParallelR, ref bool success, bool visit, int idx = -1)
    {
        //Debug.Log(node.transform.name);
        if (success == false)
        {
        ...
        else if (node.plus.GetIsEndOfParallel())
        {
            ...
            tmp.Add(calcEntireR(root, node.plus.GetEndOfParallelLink().GetComponent(), nextOfParallelR, ref success, visit, idx));
            nextOfParallelR.Add(tmp);
        ...
        if (node.plus.GetIsStartOfParallel())
        {
            ...
            for (int i = 0; i < node.plus.links.Count; i++)
            {
                sumOfParallelR += 1 / calcEntireR(root, node.plus.links[i].GetComponent(), nextOfParallelR, ref success, visit, idx);
            }
            // 각 부품의 전압, 전류를 구할 때 사용
            ParallelR tmp = new ParallelR();
            tmp.R = 1 / sumOfParallelR;
            ...
            /*            Debug.Log("병렬 연결 저항의 합 : " + 1/sumOfParallelR);*/
            // 병렬 연결 이후의 값을 전부 더함
            for (int i = 0; i < nextOfParallelR[idx].Count; i++)
            {
                totalR += nextOfParallelR[idx][i];
            }
            //nextOfParallelR[idx].Clear();

            result = node.GetR() + 1 / sumOfParallelR + totalR;
            Debug.Log(node.transform.name);
            Debug.Log(idx + "번째 병렬일때: totalR : " + totalR);
            Debug.Log(idx + "번째 병렬일때: 1/sumOfParallelR : " + 1 / sumOfParallelR);
            Debug.Log(idx + "번째 병렬일때: node.GetR() : " + node.GetR());
            return result;
            //return node.GetR() + 1 / sumOfParallelR+ totalR;
        }
        // 지금 부품이 직렬 연결이면서 방문하지 않은 부품일 때 (병렬 내부는 직렬과 같음)
        else
        {
            ...
            result = node.GetR() + calcEntireR(root, node.plus.links[0].GetComponent(), nextOfParallelR, ref success, visit, idx);
            Debug.Log(node.transform.name);
            //Debug.Log("이제까지 구한 저항 : " + result);
            return result;
            //return node.GetR() + calcEntireR(root, node.plus.links[0].GetComponent(), nextOfParallelR, ref success, visit);
    ...
    public bool circuit(ComponentClass root)
    {
        ...
        List<List<double>> listR2 = new List<List<double>>();
        ...
        r = calcEntireR(root, root, listR2, ref success, visit);
        if (success)
        {
            Debug.Log("전체 저항 : " + r);
        }
